...
export const AreaFilterBar = ({
  ...
  const handleCreateArea = async (areaName) => {
    // Check free tier limit using subscription utility
    if (!canPerformAction(userTier, 'areas', customAreas.length)) {
      alert('Free tier is limited to 2 custom areas. Upgrade to Pro for unlimited areas!');
      setShowPricing(true);
      return;
    }
