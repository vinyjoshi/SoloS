...
import { generateDateKey, getDaysInMonth, getFirstDayOfMonth, getStartOfWeek } from '../../utils/dateUtils';
    ...
    if (view === 'daily') {
      return (
        <div className="flex items-center justify-between py-4">
          <button onClick={() => changeDate(-1)} className="p-2 hover:bg-white/10 rounded-full">
            <ChevronLeft size={20} />
          </button>
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center p-2 rounded-lg transition-all ${
                  isSelected ? 'bg-white text-black' : 'hover:bg-white/5 text-zinc-500'
                }`}
              >
            ...
            {Array.from({ length: daysInMonth }, (_, i) => {
              ...
              return (
                <button
                  key={i}
                  onClick={() => setCurrentDate(d)}
                  className={`h-7 w-7 mx-auto flex items-center justify-center rounded text-xs ${
                    isSelected ? 'bg-white text-black' : 'text-zinc-500 hover:text-white'
                  }`}
                >
                  {i + 1}
                </button>
        ...
        {['daily', 'weekly', 'monthly'].map((m) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${
              view === m ? 'bg-white text-black shadow-lg' : 'text-zinc-500 hover:text-zinc-300'
            }`}
          >
            {m}
          </button>
