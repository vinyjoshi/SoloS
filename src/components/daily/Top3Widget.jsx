      ...
      <div className="space-y-3">
        {top3.map((task, idx) => (
          <div key={idx} className="flex gap-3 items-start group">
            <span
              className={`font-mono text-lg font-bold transition-colors select-none mt-1 ${
                task.done ? 'text-emerald-600' : 'text-zinc-700 group-hover:text-zinc-500'
              }`}
            >
              0{idx + 1}
            </span>
            <textarea
              ...
              className={`flex-1 bg-transparent border-b outline-none py-1.5 text-sm transition-colors resize-none overflow-hidden placeholder-zinc-800 ${
                task.done
                  ? 'text-zinc-600 line-through border-zinc-800'
                  : 'text-zinc-200 border-zinc-800 focus:border-zinc-500'
              }`}
            />
            <button
              onClick={() => {
